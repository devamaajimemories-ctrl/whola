                ...
                <div className="flex flex-col lg:flex-row gap-6">
                    ...
                    <div className="w-full lg:w-1/4 relative rounded-2xl overflow-hidden shadow-lg group min-h-[400px]">
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col justify-end p-6">
                            <div className="space-y-2 mb-6">
                                {["Adaptors, Plugs and Sockets", "Indoor Lights & Fittings", "Freezers, Refrigerators", "Audio Video System"].map((link, idx) => (
                                    <Link 
                                        key={idx} 
                                        href={`/search?q=${encodeURIComponent(link)}`} 
                                        className="block text-white/90 hover:text-white hover:translate-x-1 transition-transform"
                                    >
                                        {link}
                                    </Link>
                                ))}
                            ...
                            <Link 
                                ...
                                className="inline-block bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-6 rounded-full w-fit transition-colors shadow-md"
                            >
                                View All
                            </Link>
                        </div>
                    ...
                    <div className="w-full lg:w-3/4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {subCategories.map((cat, index) => (
                            <div key={index} className="bg-white rounded-xl p-5 shadow-sm hover:shadow-md transition-all duration-300 border border-gray-100 flex flex-col justify-between h-full group">
                                <Link href={`/search?q=${encodeURIComponent(cat.title)}`} className="flex justify-between items-start mb-3">
                                    <h3 className="font-semibold text-gray-800 line-clamp-2 flex-1 pr-2 group-hover:text-blue-600 transition-colors" title={cat.title}>
                                        {cat.title}
                                    </h3>
                                    <ArrowRight size={16} className="text-gray-400 mt-1 flex-shrink-0 group-hover:text-blue-600" />
                                </Link>
                                ...
                                <div className="flex justify-between items-start gap-3">
                                    ...
                                    <Link href={`/search?q=${encodeURIComponent(cat.title)}`} className={`w-16 h-16 rounded-lg flex items-center justify-center flex-shrink-0 ${cat.color} group-hover:scale-110 transition-transform`}>
                                        <cat.icon size={32} />
                                    </Link>
                                </div>
                            </div>
