...
interface Conversation {
    ...
    // In seller context, 'seller' field actually holds buyer info from the API 
    // (The API field naming might be generic, adjusting to data received)
    buyer?: {
        name: string;
    }
}
...
function SellerMessagesContent() {
    ...
    const [activeSellerId, setActiveSellerId] = useState<string | null>(initialBuyerId);
    ...
    const fetchConversations = async () => {
        try {
            // Depending on your API route for seller conversations
            // If using /api/chat/seller-conversations, it returns data.data
    ...
    const fetchMessages = async (buyerId: string) => {
        try {
            // Seller fetching history with a buyer
    ...
    const handleSend = async () => {
        ...
        try {
            await fetch('/api/chat/seller-send', {
                ...
                body: JSON.stringify({ buyerId: activeSellerId, message: input })
            });
            setInput("");
            fetchMessages(activeSellerId);
    ...
    // Note: The API response structure determines if we look for 'seller' or 'buyer' object
    // The types define 'seller', but in seller-conversations route we might get 'buyer'
    const activeConv = conversations.find(c => c._id === activeSellerId);
    const activeName = activeConv?.buyer?.name || activeConv?.seller?.name || "Buyer";
    const activeInitial = activeName.charAt(0);
                ...
                <div className="flex-1 overflow-y-auto bg-white">
                    {loading ? (
                        ...
                        conversations.map((conv) => {
                            const name = conv.buyer?.name || conv.seller?.name || "User";
                            return (
                                <div
                                    key={conv._id}
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                                        <div className="flex justify-between items-baseline mb-0.5">
                                            <h3 className="text-[#111b21] font-normal text-[17px] truncate">{name}</h3>
                                    ...
                                    >
                                        ...
                                        `}>
                                            ...
                                            <span className={`
                                                ...
                                                ${msg.sender === 'seller' ? 'text-[#00a884]' : 'text-[#667781]'}
                                            `}>
                                                {new Date(msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                                {msg.sender === 'seller' && <span className="ml-1">✓✓</span>}
                                            </span>
                                        </div>
                                    </div>
                                ))}
...
export default function SellerMessagesPage() {
    return (
        <Suspense fallback={<div className="flex h-screen items-center justify-center bg-[#d1d7db]"><Loader2 className="animate-spin text-[#00a884]" size={48} /></div>}>
