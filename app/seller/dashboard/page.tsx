...
export default async function SellerDashboard() {
    ...
    // Using 'any' cast here temporarily if typescript complains about IProduct interface mismatch during dev
    ...
    if (totalProducts > 0) completionScore += 30;
                                    ...
                                    <tbody className="divide-y divide-gray-100">
                                        {recentOrders.map(order => (
                                            <tr key={order._id.toString()} className="hover:bg-gray-50">
                                                ...
                                                <td className="p-4">
                                                    <span className={`px-2 py-1 rounded-full text-xs font-bold ${order.deliveryStatus === 'DELIVERED' ? 'bg-green-100 text-green-700' :
