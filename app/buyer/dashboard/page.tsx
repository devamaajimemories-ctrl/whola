...
import { headers } from "next/headers"; // Read headers set by Middleware
import User from "@/lib/models/User";
import Request from "@/lib/models/Request";
import dbConnect from "@/lib/db";
import { User as UserIcon, Phone, Mail, ShieldCheck, Clock } from 'lucide-react';
...
export default async function BuyerDashboard() {
    ...
    // 1. SECURE ID RETRIEVAL
    // We get the ID from the secure header set by Middleware, NOT the URL
    const headersList = await headers();
    const userId = headersList.get("x-user-id");

    if (!userId) return <div className="min-h-screen flex items-center justify-center">Access Denied</div>; // Should be caught by middleware anyway
        ...
        <main className="flex min-h-screen flex-col bg-gray-50">
            ...
            <div className="container mx-auto px-4 py-6">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <Link href="/buyer/messages" className="text-blue-600 hover:underline text-sm font-medium">
                        View All
                    </Link>
                </div>
                ...
                {populatedConversations.length > 0 ? (
                    ...
                    <div className="p-8 text-center bg-white rounded-xl shadow-sm border border-gray-100">
                        ...
                        <Link href="/search" className="inline-flex items-center justify-center px-6 py-2.5 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition-colors shadow-sm hover:shadow">
                            Find Sellers to Chat
                        </Link>
                    </div>
                )}
            </div>
            ...
            {activeRequests.length > 0 && (
                <div className="container mx-auto px-4 py-6">
                    <h2 className="text-2xl font-bold mb-4 text-gray-800">Your Active Requests</h2>
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                        {activeRequests.map((req: any) => (
                            <div key={req._id} className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                                <div className="flex justify-between items-start mb-2">
                                    <h3 className="font-bold text-lg text-gray-900">{req.product}</h3>
                                    <span className={`px-2 py-1 rounded text-xs font-semibold ${req.status === 'OPEN' ? 'bg-green-100 text-green-700' :
                                ...
                                <div className="text-sm text-gray-600 space-y-1 mb-4">
                                    <p>Quantity: <span className="font-medium text-gray-900">{req.quantity}</span></p>
                                    <p>Budget: <span className="font-medium text-gray-900">₹{req.budget || 'N/A'}</span></p>
                                </div>
